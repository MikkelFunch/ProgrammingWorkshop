...
public class SearchCmd {
	...
	public static void search(HashMap<String, URLList> map, String word) {
		if (!map.containsKey(word)) {
			...
			System.out.println("The word \"" + word
					+ "\" has been found in the following link(s):");
	...
	public static void main(String[] args) throws IOException {
		...
		while (true) {
			...
			if (word == null || word.length() == 0) {
				return; // If the user only pressed enter then exit the method
						// (and program)
			}
			search(map, word);
		}
	...
	public static HashMap<String, URLList> constructHashMap(
		...
		HashMap<String, URLList> map = new HashMap<String, URLList>();
		BufferedReader infile = fileReader;
		...
		while (word != null) { // Loop as long as there are new lines
			if (word.startsWith(urlPrefix)) {
				...
				map.get(word).addUrl(url); // Add url to hashset
			} else { // else add word to hashmap and create hashset with url
				URLList m = new URLList(url);
				map.put(word, m);
			}
